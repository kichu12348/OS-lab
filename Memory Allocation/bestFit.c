...
/*
this code implements the Best Fit memory allocation algorithm.
this is the non contiguous memory allocation algorithm.
it allocates the smallest available memory block that is large enough to satisfy the request.
...
3. the program iterates through each process and finds the smallest available memory block that is large enough to satisfy the request.
